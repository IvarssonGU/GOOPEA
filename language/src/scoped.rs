use crate::{
    ast::{
        ADTDefinition, AID, ConstructorDefinition, ConstructorSignature, Definition, Expression,
        FID, FunctionDefinition, FunctionSignature, MatchExpression, Pattern, Program, Type,
        UTuple, VID, write_implicit_utuple, write_indent, write_separated_list,
    },
...
fn get_scopes_same_type<'a, 'b: 'a>(
    mut iter: impl Iterator<Item = &'a ScopedExpressionNode<'b>>,
...
impl<'a> ScopedProgram<'a> {
        ...
        for def in &program.0 {
            match def {
                Definition::ADTDefinition(def) => {
                    constructor_signatures.extend(
                        def.constructors
                            .iter()
                            .map(|cons| (cons.id.clone(), &cons.arguments)),
                    );

                    all_function_signatures.extend(def.constructors.iter().map(|cons| {
                        (
                            cons.id.clone(),
                            FunctionSignature {
            ...
            match def {
                ...
                Definition::FunctionDefinition(def) => {
                    ...
                    let base_scope = def
                        ...
                        .0
                        .iter()
                        .zip(def.signature.argument_type.0.iter())
                        .map(|(vid, tp)| {
                            (
                                vid.clone(),
                                Rc::new(VariableDefinition {
                                    id: vid.clone(),
                                    tp: tp.clone(),
    ...
    pub fn get_constructor(
        &self,
        fid: &'a FID,
    ) -> Result<&ConstructorReference<'a>, CompileError<'a>> {
        self.constructors
            .get(fid)
            .ok_or_else(|| CompileError::UnknownConstructor(fid))
    }
}
...
impl<'a> ScopedExpressionNode<'a> {
    fn validate_recursively(&self, program: &'a ScopedProgram) -> CompileResult {
        ...
        match self.expr {
            Expression::FunctionCall(fid, args) => {
                self.validate_as_function_call(program, fid, args)?
            }
            Expression::Variable(vid) => {
                self.get_var(vid)?;
            }
            Expression::Match(match_expression) => {
                self.validate_as_match(program, match_expression)?
            }
    ...
    fn validate_correct_scope_children(&self) -> CompileResult {
        match self.expr {
            Expression::UTuple(_) | Expression::FunctionCall(_, _) => {
                let ScopeChildren::Many(_) = self.children else {
                    return Err(CompileError::InternalError);
                };
            }
            Expression::Integer(_) | Expression::Variable(_) => {
                let ScopeChildren::Zero = self.children else {
                    return Err(CompileError::InternalError);
                };
            }
            Expression::Match(_) => {
                let ScopeChildren::Match(_, _) = self.children else {
                    return Err(CompileError::InternalError);
                };
            }
        }
    ...
    fn validate_as_function_call(
        &self,
        program: &'a ScopedProgram,
        fid: &'a FID,
        args: &'a UTuple<Expression>,
    ) -> CompileResult {
        let expected_arg_type = &program
            .all_signatures
            .get(fid)
            .ok_or_else(|| CompileError::UnknownFunction(fid))?
            .argument_type;
        ...
        let arg_type: UTuple<Type> = UTuple(
            self.children
                .scopes()
                .iter()
                .map(|scope| scope.tp.expect_tp().map(|x| x.clone()))
                .collect::<Result<_, _>>()?,
        );
    ...
    fn validate_as_match(
        &self,
        program: &'a ScopedProgram,
        match_expression: &'a MatchExpression,
    ) -> CompileResult {
        let ScopeChildren::Match(expr_scope, case_scopes) = &self.children else {
            panic!()
        };
        get_scopes_same_type(case_scopes.iter())
            .ok_or_else(|| CompileError::MissmatchedTypes(self.expr))?;
        ...
        match &expr_scope.tp {
            ExpressionType::UTuple(_) => {
                ...
                match &match_expression.cases[0].pattern {
                    Pattern::Integer(_) | Pattern::Constructor(_, _) => {
    ...
    fn recursively_validate_fip_expression(
        &self,
        program: &'a ScopedProgram,
    ) -> Result<HashSet<&VariableDefinition>, CompileError> {
        ...
        match &self.expr {
            Expression::FunctionCall(_, _) | Expression::UTuple(_) => {
                for child in self.children.scopes() {
                    ...
                    if let Some(double_var) = used_vars.intersection(&child_used_vars).next() {
                        return Err(CompileError::FIPFunctionHasMultipleUsedVar(
                            double_var.id.clone(),
                        ));
                    }
            ...
            Expression::Integer(_) => (),
            Expression::Variable(vid) => {
                used_vars.insert(self.scope.get(vid).unwrap());
            }
            Expression::Match(expr) => {
                let ScopeChildren::Match(e1, case_scopes) = &self.children else {
                    panic!()
                };
                ...
                for (case, child) in expr.cases.iter().zip(case_scopes) {
                    ...
                    if let Some(cases_used_vars) = &cases_used_vars {
                        ...
                        if let Some(unused_var) = diff.next() {
                            return Err(CompileError::FIPFunctionHasUnusedVar(
                                unused_var.id.clone(),
                            ));
                        }
                    } else {
                ...
                if let Some(cases_used_vars) = cases_used_vars {
                    used_vars.extend(cases_used_vars);
                }
            }
        }
    ...
    fn get_var(&'a self, vid: &'a VID) -> Result<&'a Rc<VariableDefinition>, CompileError<'a>> {
        self.scope
            .get(vid)
            .ok_or_else(|| CompileError::UnknownVariable(vid))
    }
}
...
fn scope_expression<'a, 'b>(
    ...
    let (children, tp) = match expr {
        Expression::UTuple(tup) => {
            ...
            let tp = ExpressionType::UTuple(UTuple(
                children
                    ...
                    .map(|s| {
                        s.tp.tp()
                            .ok_or_else(|| CompileError::UnexpectedUTuple)
                            .map(|t| t.clone())
                    })
                    .collect::<Result<_, _>>()?,
            ));
        ...
        Expression::FunctionCall(fid, tup) => {
            ...
            let return_type = &function_signatures
                .get(fid)
                .ok_or_else(|| CompileError::UnknownFunction(fid))?
                .result_type;
            let tp = if return_type.0.len() == 1 {
                ExpressionType::Type(return_type.0[0].clone())
            } else {
                ExpressionType::UTuple(return_type.clone())
            };
            (children, tp)
        }
        ...
        Expression::Variable(var) => (
            ...
            ExpressionType::Type(
                scope
                    .get(var)
                    .ok_or_else(|| CompileError::UnknownVariable(var))?
                    .tp
                    .clone(),
            ),
        ),
        Expression::Match(match_expr) => {
            let match_on_scope = scope_expression(
                &match_expr.expr,
                scope.clone(),
                function_signatures,
                constructor_signatures,
            ...
            let case_scopes: Vec<ScopedExpressionNode<'_>> = match_expr
                .cases
                .iter()
                .map(|case| match &case.pattern {
                    Pattern::Integer(_) => scope_expression(
                        &case.body,
                        scope.clone(),
                        function_signatures,
                        constructor_signatures,
                    ...
                    Pattern::UTuple(vars) => {
                        ...
                        scope_expression(
                            ...
                            extended_scope(
                                &scope,
                                vars.0.iter().zip(types.0.iter()).map(|(new_vid, tp)| {
                                    VariableDefinition {
                                        id: new_vid.clone(),
                                        tp: tp.clone(),
                                        internal_id: get_new_internal_id(),
                    ...
                    Pattern::Constructor(fid, vars) => {
                        let cons_sig: &ConstructorSignature = constructor_signatures
                            .get(fid)
                            .ok_or(CompileError::UnknownConstructor(fid))?;
                        if cons_sig.0.len() != vars.0.len() {
                            panic!(
                                "Wrong number of arguments in match statement of case {}",
                                fid
                            );
                        }
                        ...
                        scope_expression(
                            ...
                            extended_scope(
                                &scope,
                                vars.0.iter().zip(cons_sig.0.iter()).map(|(new_vid, tp)| {
                                    VariableDefinition {
                                        id: new_vid.clone(),
                                        tp: tp.clone(),
                                        internal_id: get_new_internal_id(),
            ...
            let tp = get_scopes_same_type(case_scopes.iter())
                .ok_or_else(|| CompileError::MissmatchedTypes(expr))?;

            let children = ScopeChildren::Match(Box::new(match_on_scope), case_scopes);
...
impl Display for ExpressionType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match &self {
            ExpressionType::UTuple(utuple) => {
                write_implicit_utuple(f, &utuple.0, ", ", |f, x| write!(f, "{x}"))
            }
...
impl Display for ScopedProgram<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write_scope(f, &self.constructors, |f, x| {
            write!(f, "{}/{}", x.adt.id, x.constructor.id)
        })?;
        writeln!(f)?;

        write!(f, "// Functions: ")?;
        write_scope(f, &self.functions, |f, x| {
            write!(f, "{}[{}]", x.def.id, x.def.signature)
        })?;
        writeln!(f)?;
...
fn write_scoped_expression_node<'a>(
    f: &mut Formatter<'_>,
    node: &'a ScopedExpressionNode<'a>,
    indent: usize,
) -> std::fmt::Result {
    write_indent(f, indent)?;
    write!(f, "// type = {}, scope = ", node.tp)?;
    write_scope(f, &node.scope, |f, x| {
        write!(f, "{}|{}[{}]", x.id, x.internal_id, x.tp)
    })?;
    writeln!(f)?;
...
fn scopes_are_equal(s1: &Scope, s2: &Scope) -> bool {
    for (id, def) in s1 {
        let Some(other_def) = s2.get(id) else {
            return false;
        };
        if def != other_def {
            return false;
    ...
    for (id, def) in s2 {
        let Some(other_def) = s1.get(id) else {
            return false;
        };
        if def != other_def {
            return false;
